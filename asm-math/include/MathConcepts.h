...
namespace asmmath
{
	...
	/** @brief Partial template specialization for promoting between numeric types using `std::common_type_t`. */
	template <typename T, typename U>
	struct SafeType
	{
	...
	/** @brief Template specialization of @ref SafeType that promotes a wider integral and narrow floating point type (32-bit float) to a wider floating point type (64-bit float, or double). */
	template <typename T, typename U>
		requires std::floating_point<T> || std::floating_point<U>
	struct SafeType<T, U>
	{
		using type = std::conditional_t<(sizeof(T) > 4 || sizeof(U) > 4), double, float>;
	};
}
